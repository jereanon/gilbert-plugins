"""Tests for the Groq Whisper batch transcription backend."""
...
def backend():
    ...
    from gilbert_plugin_groq import groq_whisper

    return groq_whisper.GroqWhisperBackend()


def test_backend_is_registered():
    from gilbert_plugin_groq import groq_whisper  # noqa: F401

    assert "groq_whisper" in BatchTranscriptionBackend.registered_backends()
...
async def test_transcribe_sends_audio_and_returns_text(backend):
    ...
    assert call.kwargs["headers"]["Authorization"] == "Bearer gsk-test"


@pytest.mark.asyncio
async def test_transcribe_4xx_raises_runtime_error_with_message(backend):
    await backend.initialize({"api_key": "gsk-test"})
